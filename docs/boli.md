---
format: 
  html:
    theme: cosmo       # 使用一个干净现代的主题
    page-layout: full  # 页面宽度最大化，方便图片展示
    toc: true         # 关闭右侧目录，以免影响长图的整洁度
    embed-resources: true  # 将所有图片、样式彻底融合进单个文件
---

<h1 align="center">🪟 幕墙玻璃尺寸 🪟</h1>

***

<h2 align="center">A. 内径尺寸</h2>

![alt text](内径-长-全景.png){width="2560px"}
**🏞️ 图1. 内径-长-全景**

***

![alt text](内径-长-特写.png){width="2560px"}
**🏞️ 图2. 内径-长-特写**

***

![alt text](内径-宽-全景.png){width="2560px"}
**🏞️ 图3. 内径-宽-全景**

***

![alt text](内径-宽-特写.png){width="2560px"}
**🏞️ 图4. 内径-宽-特写**

***

<h2 align="center">B. 外径尺寸</h2>

![alt text](外径-长-全景.png){width="2560px"}
**🏞️ 图5. 外径-长-全景**

***

![alt text](外径-长-特写.png){width="2560px"}
**🏞️ 图6. 外径-长-特写**

***

![alt text](外径-宽-全景.png){width="2560px"}
**🏞️ 图7. 外径-宽-全景**

***

![alt text](外径-宽-特写.png){width="2560px"}
**🏞️ 图8. 外径-宽-特写**
